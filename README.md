...
**Stellara** 是一个面向恒星演化过程的交互式科学可视化项目。它基于 MIST v2.5（MESA Isochrones and Stellar Tracks）预计算轨迹数据，将不同初始质量、不同金属丰度恒星从主序前到各自数据终点的演化呈现在赫罗图（HR Diagram）上。
...
| 包含 | 不包含 |
|---|---|
| 单星从 PMS 到各轨迹数据终点 | 双星相互作用、合并 |
| 1–5 M☉ 轨迹中的白矮星冷却序列 | 磁场效应 |
| 高质量→碳燃烧（模型终点） | 实际超新星光变曲线 |
| Wolf-Rayet 阶段识别 | 任意旋转参数 |
| HR 图坍缩虚线（示意） | 在线 MESA 计算 |

> 当前 0.5 M☉ 子集止于亚巨星/红巨星阶段，并未延伸到白矮星冷却；高质量轨迹止于坍缩前模型终点。界面中的坍缩线和超新星外观均为示意。
